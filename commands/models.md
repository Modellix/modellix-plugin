...
3. For the request-body schema, use the public contract (JSON default, no API key):

```bash
modellix-cli model get-schema <slug>
```

If CLI is unavailable, fall back to the Docs MCP, the `docs_url` from `model describe`, or the matching entry in https://docs.modellix.ai/llms.txt.
